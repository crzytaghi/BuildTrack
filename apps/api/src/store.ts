...
const SEED_COMPANY_ID = 'company_seed';
const SEED_SALT = 'seed_salt_fixed';

export const seed = async () => {
  const existing = await prisma.company.findUnique({ where: { id: SEED_COMPANY_ID } });
  if (existing) return;

  await prisma.company.create({
    data: { id: SEED_COMPANY_ID, name: 'BuildTrack Demo Co.', companySetupComplete: true },
  });

  await prisma.user.create({
    data: {
      id: 'user_seed',
      email: 'seed@buildtrack.dev',
      name: 'Seed User',
      passwordHash: crypto.pbkdf2Sync('securepass1', SEED_SALT, 120000, 64, 'sha512').toString('hex'),
  ...
  const projectDefs = [
    { id: 'proj_1', name: 'Maple St Renovation',     status: 'active',    startDate: '2026-01-15', budgetTotal: 420000 },
    { id: 'proj_2', name: 'Harbor View Condos',       status: 'active',    startDate: '2025-11-01', budgetTotal: 2800000 },
    { id: 'proj_3', name: 'Downtown Office Complex',  status: 'planning',  startDate: '2026-04-01', budgetTotal: 5100000 },
    { id: 'proj_4', name: 'Riverside Warehouse',      status: 'on_hold',   startDate: '2025-09-15', budgetTotal: 875000 },
    { id: 'proj_5', name: 'Elmwood Medical Center',   status: 'active',    startDate: '2026-02-01', budgetTotal: 9200000 },
    { id: 'proj_6', name: 'Sunset Retail Strip',      status: 'planning',  startDate: '2026-06-01', budgetTotal: 1350000 },
    { id: 'proj_7', name: 'Lakeshore Apartments',     status: 'completed', startDate: '2025-03-01', endDate: '2025-12-20', budgetTotal: 3400000 },
    { id: 'proj_8', name: 'Tech Park Building B',     status: 'active',    startDate: '2026-01-05', budgetTotal: 6750000 },
  ];

  await prisma.project.createMany({ data: projectDefs.map(p => ({ ...p, companyId: SEED_COMPANY_ID })) });

  const taskCounts = [12, 5, 38, 21, 9, 40, 17, 28];
  const taskData: { id: string; projectId: string; title: string; status: string; dueDate: string }[] = [];
  projectDefs.forEach((proj, i) => {
    for (let t = 0; t < taskCounts[i]; t++) {
      taskData.push({
        id: `task_${proj.id}_${t + 1}`,
        projectId: proj.id,
  ...
  await prisma.category.createMany({
    data: [
      { id: 'cat_1', name: 'Materials',       companyId: SEED_COMPANY_ID },
      { id: 'cat_2', name: 'Labor',            companyId: SEED_COMPANY_ID },
      { id: 'cat_3', name: 'Equipment',        companyId: SEED_COMPANY_ID },
      { id: 'cat_4', name: 'Subcontractors',   companyId: SEED_COMPANY_ID },
      { id: 'cat_5', name: 'Permits & Fees',   companyId: SEED_COMPANY_ID },
      { id: 'cat_6', name: 'Other',            companyId: SEED_COMPANY_ID },
    ],
  });

  await prisma.vendor.createMany({
    data: [
      { id: 'vendor_1',  name: 'Concrete Supply Co.',    trade: 'Materials',      companyId: SEED_COMPANY_ID },
      { id: 'vendor_2',  name: 'Rodriguez Framing LLC',  trade: 'Labor',          companyId: SEED_COMPANY_ID },
      { id: 'vendor_3',  name: 'City of Springfield',    trade: 'Permits & Fees', companyId: SEED_COMPANY_ID },
      { id: 'vendor_4',  name: 'Pacific Electrical LLC', trade: 'Electrical',     companyId: SEED_COMPANY_ID },
      { id: 'vendor_5',  name: 'Heavy Lift Rentals',     trade: 'Equipment',      companyId: SEED_COMPANY_ID },
      { id: 'vendor_6',  name: 'MedTech HVAC Systems',   trade: 'HVAC',           companyId: SEED_COMPANY_ID },
      { id: 'vendor_7',  name: 'CoreSteel Fabricators',  trade: 'Steel',          companyId: SEED_COMPANY_ID },
      { id: 'vendor_8',  name: 'Atlas Equipment Rentals', trade: 'Equipment',     companyId: SEED_COMPANY_ID },
      { id: 'vendor_9',  name: 'Western Lumber Co.',     trade: 'Materials',      companyId: SEED_COMPANY_ID },
      { id: 'vendor_10', name: 'Summit Roofing Inc.',    trade: 'Labor',          companyId: SEED_COMPANY_ID },
    ],
  });
